<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jstl/core_rt"%>
<!doctype html>
<html lang="ko">
<head>
<jsp:include page="header.jsp"></jsp:include>
<meta charset="UTF-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta property="og:title" content="YES24 티켓 - 뮤지컬">
<meta property="og:type" content="website">
<meta property="og:site_name" content="YES24 티켓">
<meta property="og:description">
<meta property="fb:app_id">
<title>YES24 티켓</title>
<script type="text/javascript"
	src="/web/resources/ticketMain2/New/Js/jquery-3.4.1_min.js"></script>
<!-- Google Tag Manager -->

<!-- End Google Tag Manager -->
<script type="text/javascript"
	src="/web/resources/ticketMain2/New/Js/swiper_min.js"></script>
<script type="text/javascript"
	src="/web/resources/ticketMain2/New/Js/jquery.lazy.min.js"></script>
<script type="text/javascript"
	src="/web/resources/ticketMain2/New/Js/Main.js?v=391"></script>


<link rel="stylesheet" type="text/css"
	href="/web/resources/ticketMain2/New/Css/reset.css">
<link rel="stylesheet" type="text/css"
	href="/web/resources/ticketMain2/New/Css/swiper.css">
<link rel="stylesheet" type="text/css"
	href="/web/resources/ticketMain2/New/Css/jquery.mCustomScrollbar.css">
<link rel="stylesheet" type="text/css"
	href="/web/resources/ticketMain2/New/Css/common.css?v=17">
<link rel="stylesheet" type="text/css"
	href="/web/resources/ticketMain2/New/Css/main2.css?v=2020012203">

<link type="text/css" rel="stylesheet" href="/web/resources/final_style/css/bootstrap.css">
<link rel='stylesheet' type='text/css'
	href='/web/resources/ticketMain2/New/Css/reset.css'>
<link rel='stylesheet' type='text/css'
	href='/web/resources/ticketMain2/New/Css/swiper.css'>
<link rel='stylesheet' type='text/css'
	href='/web/resources/ticketMain2/New/Css/jquery.mCustomScrollbar.css'>
<link rel='stylesheet' type='text/css'
	href='/web/resources/ticketMain2/New/Css/common.css?v=17'>
<link rel='stylesheet' type='text/css'
	href='/web/resources/ticketMain2/New/Css/main2.css?v=2020012203'>

<div class="m2-kv-hidden">   <!-- 상단 돌아가는 컨텐츠 -->
		<div class="m2-kv-wrap">
			<div class="m2-kv">
				<div class="swiper-container m2-kvs">
					<div class="swiper-wrapper">
					<c:forEach var="i" items="${musicallist}" varStatus="cnt">
						<div class="swiper-slide">
							<a href="/web/contentImg?no=${i.no}"
								target="_self" title="${i.title}" id="img"><img id="imgsrc"
								src="${i.imgsrc}"
								alt="">
							<div class="m2-kvs-txt">
									<p class="m2-kvs-tit">${i.ptitle }</p>
									<p class="m2-kvs-detail">${i.detail}</p>
								</div></a>
						</div>
						</c:forEach>
<!-- 						<div class="swiper-slide"> -->
<!-- 							<a href="#" -->
<%-- 								target="_self" title="연극 <한뼘사이>"><img --%>
<!-- 								src="http://tkfile.yes24.com/upload2/PerfBlog/202201/20220126/20220126-38677.jpg" -->
<%-- 								alt="연극 <한뼘사이>"> --%>
<!-- 							<div class="m2-kvs-txt"> -->
<!-- <!-- 									<p class="m2-kvs-tit">연극 <한뼘사이></p> --> 
<!-- <!-- 									<p class="m2-kvs-detail">OPEN RUN 대학로 라온아트홀</p> --> 
<!-- 								</div></a> -->
<!-- 						</div> -->
<!-- 						<div class="swiper-slide"> -->
<!-- 							<a -->
<!-- 								href="#" -->
<!-- 								target="_self" title="연극 [나의PS파트너]"><img -->
<!-- 								src="http://tkfile.yes24.com/upload2/PerfBlog/202210/20221011/20221011-29162.jpg" -->
<!-- 								alt="연극 [나의PS파트너]"> -->
<!-- 							<div class="m2-kvs-txt"> -->
<!-- <!-- 									<p class="m2-kvs-tit"> --> 
<!-- <!-- 										연극 [나의PS파트너] --> 
<!-- <!-- 									</p> --> 
<!-- <!-- 									<p class="m2-kvs-detail">OPEN RUN 대학로 마루아트홀</p> --> 
<!-- 								</div></a> -->
<!-- 						</div> -->
<!-- 						<div class="swiper-slide"> -->
<!-- 							<a href="#" target="_self" -->
<%-- 								title="연극 <세상친구>"><img --%>
<!-- 								src="http://tkfile.yes24.com/upload2/PerfBlog/202302/20230202/20230202-44861.jpg" -->
<%-- 								alt="연극 <세상친구>"> --%>
<!-- 							<div class="m2-kvs-txt"> -->
<!-- <!-- 									<p class="m2-kvs-tit"> --> 
<!-- <!-- 										연극 <세상친구> --> 
<!-- <!-- 									</p> --> 
<!-- <!-- 									<p class="m2-kvs-detail">2023.02.23 ~ 2023.03.12 자유극장</p> --> 
<!-- 								</div></a> -->
<!-- 						</div> -->
<!-- 						<div class="swiper-slide"> -->
<!-- 							<a -->
<!-- 								href="#" -->
<%-- 								target="_self" title="연극 <셰익스피어 인 러브>"><img --%>
<!-- 								src=http://tkfile.yes24.com/upload2/PerfBlog/202212/20221215/20221215-44392.jpg -->
<%-- 								alt="연극 <셰익스피어 인 러브>"> --%>
<!-- 							<div class="m2-kvs-txt"> -->
<!-- <!-- 									<p class="m2-kvs-tit"> --> 
<!-- <!-- 										연극 <셰익스피어 인 러브> --> 
<!-- <!-- 									</p> --> 
<!-- <!-- 									<p class="m2-kvs-detail">2023.01.28 ~ 2023.03.26 예술의전당 CJ 토월극장</p> --> 
<!-- 								</div></a> 
<!-- 						</div> -->
<!-- 						<div class="swiper-slide"> -->
<!-- 							<a href="#" target="_self" -->
<%-- 								title="연극 <포쉬>"><img --%>
<!-- 								src="http://tkfile.yes24.com/upload2/PerfBlog/202302/20230214/20230214-45012.jpg" -->
<%-- 								alt="연극 <포쉬>"> --%>
<!-- 							<div class="m2-kvs-txt"> -->
<!-- <!-- 									<p class="m2-kvs-tit" fontcolor="black"> --> 
<!-- <!-- 										연극 <포쉬> --> 
<!-- <!-- 									</p> --> 
<!-- <!-- 									<p class="m2-kvs-detail">2023.03.09 ~ 2023.05.21 예스24스테이지 3관</p> --> 
<!-- 								</div></a> 
<!-- 						</div> -->
<!-- 						<div class="swiper-slide"> -->
<!-- 							<a href="#" target="_self" -->
<%-- 								title="연극 <비누향기>"><img --%>
<!-- 								src="http://tkfile.yes24.com/upload2/perfblog/202201/20220127/20220127-40603.jpg/dims/quality/70/" -->
<!-- 								alt=""> -->
<!-- 							<div class="m2-kvs-txt"> -->
<!-- <!-- 									<p class="m2-kvs-tit"> --> 
<!-- <!-- 										뮤지컬 --> 
<!-- <!-- 										<베토벤></베토벤> --> 
<!-- <!-- 									</p> --> 
<!-- <!-- 									<p class="m2-kvs-detail">2023.01.12 ~ 2023.03.26 예술의전당 --> -->
<!-- <!-- 										오페라극장</p> --> 
<!-- 								</div></a> 
<!-- 						</div> -->
<!-- 						<div class="swiper-slide"> -->
<!-- 							<a href="/Perf/44728?Gcode=009_400" target="_self" -->
<%-- 								title="연극 <사춘기 메들리>"><img --%>
<!-- 								src="http://tkfile.yes24.com/upload2/perfblog/202303/20230306/20230306-29266.jpg/dims/quality/70/" -->
<!-- 								alt=""> -->
<!-- 							<div class="m2-kvs-txt"> -->
<!-- <!-- 									<p class="m2-kvs-tit">디 오리지널 프렌치 뮤지컬 콘서트</p> --> 
<!-- <!-- 									<p class="m2-kvs-detail">2023.03.18 ~ 2023.03.19 세종문화회관 대극장</p> --> 
<!-- 								</div></a> -->
<!-- 						</div> -->
<!-- 						<div class="swiper-slide"> -->
<!-- 							<a -->
<!-- 								href="GenreBridge.aspx-3.html?genre=15457&id=1103&Gcode=009_400&Gcode=009_400&Gcode=009_400" -->
<!-- 								target="_self" title="[인천] 뮤지컬 [브로드웨이 42번가]"><img -->
<!-- 								src="/web/resources/ticketMain2/Upload2/Display/202210/20221007/gmain_top_43616.jpg/dims/quality/70/index.htm" -->
<!-- 								alt="뮤지컬 <브로드웨이 42번가>"> -->
<!-- 							<div class="m2-kvs-txt"> -->
<!-- 									<p class="m2-kvs-tit"> -->
<!-- 										뮤지컬 -->
<!-- 										<브로드웨이 42번가=""></브로드웨이> -->
<!-- 									</p> -->
<!-- 									<p class="m2-kvs-detail">2022.11 ~ 2023.04 전국투어</p> -->
<!-- 								</div></a> -->
<!-- 						</div> -->
					</div>
				</div>
			</div>
		</div>
		<div class="swiper-button-next m2-kvs"></div>
		<div class="swiper-button-prev m2-kvs"></div>
	</div>


<section class="m2-sec02">  <!-- WHAT"S HOT 시작 -->
		<p class="m2-tit">   <!-- WHAT"S HOT 이미지 -->
			<img src="http://tkfile.yes24.com/imgNew/genre/m2-tit02.png"
				alt="">
		</p>   <!-- /WHAT"S HOT 이미지 -->
		<div class="ms-list-imgs">   <!-- 첫줄 5개의 컨텐츠 DIV 5개씩 묶고 끊기 -->
		<c:forEach var="i" items="${musicallist2}" varStatus="cnt">
		<a href="/web/contentImg?no=${i.no}" target="_self"
				title="${i.title}" id="img"><div class="list-bigger-wrap">
					<img class="lazyload" id="imgsrc"
						data-src="${i.imgsrc}"
						alt="">
					<div class="list-bigger-txt">
						<p class="list-b-tit1">
							${i.ptitle}
						</p>
<!-- 						<p class="list-b-tit2">프리뷰 할인 30%</p> -->
<!-- 						<p class="list-b-circle">30%</p> -->
					</div>
				</div></a>
				</c:forEach>
<%-- 			<a href="#" target="_self" title="연극 <연애하기 좋은날>"><div --%>
<!-- 					class="list-bigger-wrap"> -->
<!-- 					<img class="lazyload" -->
<!-- 						data-src="http://tkfile.yes24.com/upload2/perfblog/202302/20230220/20230220-34882.jpg/dims/quality/70/" -->
<%-- 						alt="연극 <연애하기 좋은날>"> --%>
<!-- 					<div class="list-bigger-txt"> -->
<!-- 						<p class="list-b-tit1"> -->
<!-- 							연극 <연애하기 좋은날> -->
<!-- 						</p> -->
<!-- <!-- 						<p class="list-b-tit2">조기예매 할인 30%</p> --> 
<!-- <!-- 						<p class="list-b-circle">30%</p> --> 
<!-- 					</div> -->
<!-- 				</div></a><a href="#" target="_self" -->
<%-- 				title="연극 <헝키>"><div class="list-bigger-wrap"> --%>
<!-- 					<img class="lazyload" -->
<!-- 						data-src="http://tkfile.yes24.com/upload2/perfblog/202301/20230118/20230118-44767.jpg/dims/quality/70/" -->
<%-- 						alt="연극 <헝키>"> --%>
<!-- 					<div class="list-bigger-txt"> -->
<!-- 						<p class="list-b-tit1"> -->
<!-- 							연극 <헝키> -->
<!-- 						</p> -->
<!-- <!-- 						<p class="list-b-tit2">조기예매 할인 20%</p> --> 
<!-- <!-- 						<p class="list-b-circle">20%</p> --> 
<!-- 					</div> -->
<!-- 				</div></a><a href="#" target="_self" -->
<%-- 				title="연극 <옥탑방 고양이>"><div class="list-bigger-wrap"> --%>
<!-- 					<img class="lazyload" -->
<!-- 						data-src="http://tkfile.yes24.com/upload2/perfblog/202110/20211001/20211001-40181.jpg/dims/quality/70/" -->
<%-- 						alt="연극 <옥탑방 고양이>"> --%>
<!-- 					<div class="list-bigger-txt"> -->
<!-- 						<p class="list-b-tit1"> -->
<!-- 							연극 <옥탑방 고양이> -->
<!-- 						</p> -->
<!-- <!-- 						<p class="list-b-tit2">프리뷰 할인 30%</p> --> 
<!-- <!-- 						<p class="list-b-circle">30%</p> --> 
<!-- 					</div> -->
<!-- 				</div></a><a href="#" target="_self" -->
<%-- 				title="연극 <신바람 난 삼대>"><div class="list-bigger-wrap"> --%>
<!-- 					<img class="lazyload" -->
<!-- 						data-src="http://tkfile.yes24.com/upload2/perfblog/202301/20230131/20230131-40227.jpg/dims/quality/70/" -->
<%-- 						alt="연극 <신바람 난 삼대>"> --%>
<!-- 					<div class="list-bigger-txt"> -->
<!-- 						<p class="list-b-tit1"> -->
<!-- 							연극 <신바람 난 삼대> -->
<!-- 						</p> -->
<!-- <!-- 						<p class="list-b-tit2">프리뷰 할인 30%</p> --> 
<!-- <!-- 						<p class="list-b-circle">30%</p> --> 
<!-- 					</div> -->
<%-- 				</div></a><a href="#" target="_self" title="연극 <라면>"><div --%>
<!-- 					class="list-bigger-wrap"> -->
<!-- 					<img class="lazyload" -->
<!-- 						data-src="http://tkfile.yes24.com/upload2/perfblog/202103/20210316/20210316-38572.jpg/dims/quality/70/" -->
<%-- 						alt="연극 <라면>"> --%>
<!-- 					<div class="list-bigger-txt"> -->
<!-- 						<p class="list-b-tit1"> -->
<!-- 							연극 <라면> -->
<!-- 						</p> -->
<!-- <!-- 						<p class="list-b-tit2">All For One 할인 25%</p> --> 
<!-- <!-- 						<p class="list-b-circle">25%</p> --> 
<!-- 					</div> -->
<!-- 				</div></a> -->
		</div>   <!-- /첫줄 5개의 컨텐츠 DIV 5개씩 묶고 끊기 -->
		<div class="ms-list-imgs">  <!-- 둘째줄 5개의 컨텐츠 DIV 5개씩 묶고 끊기 -->
		<c:forEach var="i" items="${musicallist3}" varStatus="cnt">
		<a href="/web/contentImg?no=${i.no}" target="_self"
				title="${i.title}" id="img"><div class="list-bigger-wrap">
					<img class="lazyload" id="imgsrc"
						data-src="${i.imgsrc}"
						alt="">
					<div class="list-bigger-txt">
						<p class="list-b-tit1">
							${i.ptitle}
						</p>
<!-- 						<p class="list-b-tit2">프리뷰 할인 30%</p> -->
<!-- 						<p class="list-b-circle">30%</p> -->
					</div>
				</div></a>
				</c:forEach>
<%-- 			<a href="#" target="_self" title="연극 <행오버>"><div --%>
<!-- 			 class="list-bigger-wrap"> -->
<!-- 					<img class="lazyload" -->
<!-- 						data-src="http://tkfile.yes24.com/upload2/perfblog/202203/20220324/20220324-41222.jpg/dims/quality/70/" -->
<%-- 				alt="연극 <행오버>"> --%>
<!-- 					<div class="list-bigger-txt"> -->
<!-- 						<p class="list-b-tit1"> -->
<!-- 							연극 <행오버> -->
<!-- 						</p> -->
<!-- <!-- 						<p class="list-b-tit2">재관람 할인 30%</p> --> 
<!-- <!-- 						<p class="list-b-circle">30%</p> --> 
<!-- 					</div> -->
<%-- 				</div></a><a href="#" target="_self" title="연극 <키스>"><div --%>
<!-- 					class="list-bigger-wrap"> -->
<!-- 					<img class="lazyload" -->
<!-- 						data-src="http://tkfile.yes24.com/upload2/perfblog/202302/20230207/20230207-44948.jpg/dims/quality/70/" -->
<%-- 						alt="연극 <키스>"> --%>
<!-- 					<div class="list-bigger-txt"> -->
<!-- 						<p class="list-b-tit1"> -->
<!-- 							연극 <키스> -->
<!-- 						</p> -->
<!-- <!-- 						<p class="list-b-tit2">입문 할인 50%</p> --> 
<!-- <!-- 						<p class="list-b-circle">50%</p> --> 
<!-- 					</div> -->
<!-- 				</div></a><a href="#" target="_self" -->
<%-- 				title="연극 <2호선 세입자>"><div class="list-bigger-wrap"> --%>
<!-- 					<img class="lazyload" -->
<!-- 						data-src="http://tkfile.yes24.com/upload2/perfblog/202201/20220113/20220113-41225.jpg/dims/quality/70/" -->
<%-- 						alt="연극 <2호선 세입자>"> --%>
<!-- 					<div class="list-bigger-txt"> -->
<!-- 						<p class="list-b-tit1"> -->
<!-- 							연극 <2호선 세입자> -->
<!-- 						</p> -->
<!-- <!-- 						<p class="list-b-tit2">재관람 할인 20%</p> --> 
<!-- <!-- 						<p class="list-b-circle">20%</p> --> 
<!-- 					</div> -->
<!-- 				</div></a><a href="#" target="_self" -->
<!-- 				title="연극 <친정엄마와 2박 3일>"> -->
<!-- 				<div class="list-bigger-wrap"> -->
<!-- 					<img class="lazyload" -->
<!-- 						data-src="http://tkfile.yes24.com/upload2/perfblog/202301/20230104/20230104-44603_1.jpg/dims/quality/70/" -->
<!-- 						alt="연극 <친정엄마와 2박 3일>"> -->
<!-- 					<div class="list-bigger-txt"> -->
<!-- 						<p class="list-b-tit1"> -->
<!-- 							연극 <친정엄마와 2박 3일> -->
<!-- 						</p> -->
<!-- <!-- 						<p class="list-b-tit2">조기예매 할인 20%</p> --> 
<!-- <!-- 						<p class="list-b-circle">20%</p> --> 
<!-- 					</div> -->
<!-- 				</div> -->
<%-- 			</a><a href="#" target="_self" title="연극 <수상한 흥신소>"><div --%>
<!-- 					class="list-bigger-wrap"> -->
<!-- 					<img class="lazyload" -->
<!-- 						data-src="http://tkfile.yes24.com/upload2/perfblog/202111/20211105/20211105-40695.jpg/dims/quality/70/" -->
<%-- 						alt="연극 <수상한 흥신소>"> --%>
<!-- 					<div class="list-bigger-txt"> -->
<!-- 						<p class="list-b-tit1"> -->
<!-- 							연극 <수상한 흥신소> -->
<!-- 						</p> -->
<!-- <!-- 						<p class="list-b-tit2">R석 할인 25%</p> --> 
<!-- <!-- 						<p class="list-b-circle">25%</p> --> 
<!-- 					</div> -->
<!-- 				</div></a> -->
		</div>   <!-- /둘째줄 5개의 컨텐츠 DIV 5개씩 묶고 끊기 -->
	</section>   <!-- WHAT"S HOT 전체   -->




<section class="m2-sec05">   
		<div class="ms5-wrap">  
			<div class="ms5-l"> <!-- WEEKLY RANKING 시작 -->
				<p class="ms5-l-tit" id="ranking">
					WEEKLY RANKING 
<!-- 					<a href="/Rank/Musical"><img -->
<!-- 						src="/web/resources/ticket.yes24.com/imgNew/genre/ms5-more.png" -->
<!-- 						alt=""></a> -->
				</p>
				<ul>
				<c:forEach var="i" items="${musicallist4}">
				<li><a
						href='/web/contentImg?no=${i.no}'
						title='${i.title}'><dl>
								<dt>
									${i.ranking }<span class='dash'>-</span>
								</dt>
								<dd>
									<img
										src='${i.imgsrc}'
										alt=''>
								</dd>
								<dd class='ms5-l-txt'>
									<p class='ms5-l-txt1'>${i.ptitle}</p>
									<p class='ms5-l-txt2'>
										${i.detail}
									</p>
								</dd>
							</dl></a></li>
							</c:forEach>
<!-- 					<li class="on"><a -->
<!-- 						href="#" -->
<!-- 						title="연극 〈셰익스피어 인 러브〉"><dl> -->
<!-- 								<dt> -->
<!-- 									1위<span class="dash">-</span> -->
<!-- 								</dt> -->
<!-- 								<dd> -->
<!-- 									<img -->
<!-- 										src="http://tkfile.yes24.com/upload2/perfblog/202212/20221215/20221215-44392.jpg/dims/quality/70/" -->
<!-- 										alt=""> -->
<!-- 								</dd> -->
<!-- 								<dd class="ms5-l-txt"> -->
<!-- 									<p class="ms5-l-txt1">연극 〈셰익스피어 인 러브〉</p> -->
<!-- 									<p class="ms5-l-txt2"> -->
<!-- 										2023.01.28 ~ 2023.03.26<br>예술의전당 CJ 토월극장 -->
<!-- 									</p> -->
<!-- 								</dd> -->
<!-- 							</dl></a></li> -->
<!-- 					<li><a -->
<!-- 						href="#" -->
<!-- 						title="연극 [무인도 탈출기]"><dl> -->
<!-- 								<dt> -->
<!-- 									2위<span class="new">new</span> -->
<!-- 								</dt> -->
<!-- 								<dd> -->
<!-- 									<img -->
<!-- 										src="http://tkfile.yes24.com/upload2/perfblog/202302/20230221/20230221-45072.jpg/dims/quality/70/" -->
<!-- 										alt=""> -->
<!-- 								</dd>  -->
<!-- 								<dd class="ms5-l-txt"> -->
<!-- 									<p class="ms5-l-txt1">연극 [무인도 탈출기]</p> -->
<!-- 									<p class="ms5-l-txt2"> -->
<!-- 										2023.03.16 ~ 2023.03.26<br>대학로 자유극장 -->
<!-- 									</p> -->
<!-- 								</dd> -->
<!-- 							</dl></a></li> -->
<!-- 					<li><a -->
<!-- 						href="#" -->
<!-- 						title="연극 〈포쉬〉"><dl> -->
<!-- 								<dt> -->
<!-- 									3위<span class="down">1</span> -->
<!-- 								</dt> -->
<!-- 								<dd> -->
<!-- 									<img -->
<!-- 										src="http://tkfile.yes24.com/upload2/perfblog/202302/20230214/20230214-45012.jpg/dims/quality/70/" -->
<!-- 										alt=""> -->
<!-- 								</dd> -->
<!-- 								<dd class="ms5-l-txt"> -->
<!-- 									<p class="ms5-l-txt1">연극 〈포쉬〉</p> -->
<!-- 									<p class="ms5-l-txt2"> -->
<!-- 										2023.03.09 ~ 2023.05.21<br>예스24스테이지 3관 -->
<!-- 									</p> -->
<!-- 								</dd> -->
<!-- 							</dl></a></li> -->
<!-- 					<li><a -->
<!-- 						href="#" -->
<!-- 						title="음악극 올드위키드송 (Old Wicked Songs)"><dl> -->
<!-- 								<dt> -->
<!-- 									4위<span class="down">1</span> -->
<!-- 								</dt> -->
<!-- 								<dd> -->
<!-- 									<img -->
<!-- 										src="http://tkfile.yes24.com/upload2/perfblog/202302/20230213/20230213-44048.jpg/dims/quality/70/" -->
<!-- 										alt=""> -->
<!-- 								</dd> -->
<!-- 								<dd class="ms5-l-txt"> -->
<!-- 									<p class="ms5-l-txt1">음악극 올드위키드송 (Old Wicked Songs)</p> -->
<!-- 									<p class="ms5-l-txt2"> -->
<!-- 										2022.12.10 ~ 2023.02.19<br>예스24스테이지 2관 -->
<!-- 									</p> -->
<!-- 								</dd> -->
<!-- 							</dl></a></li> -->
<!-- 					<li><a -->
<!-- 						href="#" -->
<!-- 						title="다큐멘터리음악극 [1919 필라델피아] - 창원초청공연"><dl> -->
<!-- 								<dt> -->
<!-- 									5위<span class="down">1</span> -->
<!-- 								</dt> -->
<!-- 								<dd> -->
<!-- 									<img -->
<!-- 										src="http://tkfile.yes24.com/upload2/perfblog/202301/20230110/20230110-44645.jpg/dims/quality/70/" -->
<!-- 										alt=""> -->
<!-- 								</dd> -->
<!-- 								<dd class="ms5-l-txt"> -->
<!-- 									<p class="ms5-l-txt1">다큐멘터리음악극 [1919 필라델피아] - 창원초청공연</p> -->
<!-- 									<p class="ms5-l-txt2"> -->
<!-- 										2023.03.01 ~ 2023.03.05<br>KBS창원홀 -->
<!-- 									</p> -->
<!-- 								</dd> -->
<!-- 							</dl></a></li> -->
				</ul>
			</div>   <!-- /WEEKLY RANKING -->

		<div class="ms5-r">   <!-- 지역별 추천 3개 -->
				<p class="ms5-r-tit">
					<span>지역별 추천</span>
				</p>
				<div class="ms5-rs-wrap">   <!-- 지역별 추천 이미지 -->
					<div class="swiper-container ms5-rs">   
						<div class="swiper-wrapper">   <!-- 지역별 추천 컨텐츠 -->
						<c:forEach var="i" items="${musicallist5}">
							<div class="swiper-slide">
								<a href="/web/contentImg?no=${i.no}" target="_self"
									title="${i.title}" id="img"><span><img id="imgsrc"
										src="${i.imgsrc}"
										alt=""></span>
								<div>
										<p class="ms5-r-txt1">
											${i.ptitle}
										</p>
<!-- 										<p class="ms5-r-txt2">학생 할인 30%</p> -->
									</div>
									<p>${i.detail}</p></a>
							</div>
							</c:forEach>
<!-- 							<div class="swiper-slide"> -->
<!-- 								<a href="#" target="_self" -->
<%-- 									title="뮤지컬 <드라큘라>"><span><img --%>
<!-- 										src="/web/resources/ticket.yes24.com/Upload2/Display/202210/20221021/gmain_top_43815.jpg/dims/quality/70/index.htm" -->
<%-- 										alt="뮤지컬 <드라큘라>"></span> --%>
<!-- 								<div> -->
<!-- 										<p class="ms5-r-txt1"> -->
<!-- 											뮤지컬 -->
<!-- 											<드라큘라></드라큘라> -->
<!-- 										</p> -->
<!-- <!-- 										<p class="ms5-r-txt2">학생 할인 30%</p> --> 
<!-- 									</div> 
<!-- 									<p>송파구</p></a> -->
<!-- 							</div> -->
<!-- 							<div class="swiper-slide"> -->
<!-- 								<a href="#" target="_self" -->
<%-- 									title="뮤지컬 <미드나잇 : 앤틀러스>"><span><img --%>
<!-- 										src="http://tkfile.yes24.com/upload2/perfblog/202301/20230118/20230118-44374.jpg/dims/quality/70/" -->
<%-- 										alt="뮤지컬 <미드나잇 : 앤틀러스>"></span> --%>
<!-- 								<div> -->
<!-- 										<p class="ms5-r-txt1"> -->
<!-- 											뮤지컬 -->
<!-- 											<미드나잇 : 앤틀러스></미드나잇 : 앤틀러스> -->
<!-- 										</p> -->
<!-- <!-- 										<p class="ms5-r-txt2">프리뷰 할인 30%</p> --> 
<!-- 									</div> -->
<!-- 									<p>종로구</p> -->
<!-- 									</a> -->
<!-- 							</div> -->
<!-- 							<div class="swiper-slide"> -->
<!-- 								<a href="#" target="_self" -->
<!-- 									title="뮤지컬 <은밀하게 위대하게： THE LAST> "><span><img -->
<!-- 										src="http://tkfile.yes24.com/upload2/perfblog/202302/20230201/20230201-44819.jpg/dims/quality/70/" -->
<!-- 										alt="뮤지컬 <은밀하게 위대하게： THE LAST> "></span> -->
<!-- 								<div> -->
<!-- 										<p class="ms5-r-txt1"> -->
<!-- 											뮤지컬 -->
<!-- 											<은밀하게 위대하게： THE LAST></은밀하게 위대하게： THE LAST> -->
<!-- 										</p> -->
<!-- <!-- 										<p class="ms5-r-txt2">청소년 할인 30%</p> --> 
<!-- 									</div> -->
<!-- 									<p>강남구</p></a> -->
<!-- 							</div> -->
						</div>   <!-- 지역별 추천 컨텐츠 -->
					</div>   
					<div class="swiper-button-next ms5-rs"></div>
					<div class="swiper-button-prev ms5-rs"></div>
				</div>   <!-- /지역별 추천 이미지 -->
			</div>   <!-- /지역별 추천 3개 -->
		</div>
	</section>

<script type="text/javascript">
		$(function() {
			$(".lazyload").Lazy(
					{
						effect : "fadeIn",
						effectTime : 200,
						defaultImage : "http://tkfile.yes24.com"
								+ "/imgNew/common/noimg.gif"
					});
		});

		var ua = window.navigator.userAgent.toLowerCase();
		if (/iphone/.test(ua) || /android/.test(ua) || /opera/.test(ua)
				|| /bada/.test(ua)) {
			if (confirm("모바일 웹서비스 이용이 가능합니다.\r\n모바일웹으로 이동하시겠습니까? ")) {
				location.href = "http://m.ticket.yes24.com";
			}
		}
	</script>
</body>
</html>